...
interface MenuSuccessModalProps {
    isOpen: boolean;
    onClose: () => void;
    mode: 'add' | 'update';
}

export default function MenuSuccessModal({ isOpen, onClose, mode }: MenuSuccessModalProps) {
    if (!isOpen) return null;

    return (
        <div className="fixed inset-0 z-[60] flex items-center justify-center bg-black/40 p-4 backdrop-blur-sm">
            <div className="bg-white rounded-[2.5rem] shadow-2xl w-full max-w-lg overflow-hidden p-12 flex flex-col items-center text-center">
                {/* Success Icon */}
                ...
                <h2 className="text-3xl font-bold text-gray-900 mb-10 leading-tight px-4">
                    Anda berhasil {mode === 'add' ? 'menambahkan' : 'update'} menu!
                </h2>
